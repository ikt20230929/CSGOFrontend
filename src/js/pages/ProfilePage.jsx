...
export default function ProfilePage() {
  ...
  return (
    <div>
      ...
      gradient={{ from: 'rgba(255, 255, 255, 1)', to: 'rgba(143, 143, 143, 1)', deg: 90  }}>{profileData.profile.username} profilja</Text>
        </Group>
        <Space h="sm"/>
...
<Modal opened={opened} onClose={close} title="Adataid szerkesztése" transitionProps={{ transition: 'fade', duration: 600, timingFunction: 'linear' }}>
      ...
      variant="filled"
      label="Profilkép"
      description="Válassz magadnak egy új profilképet!"
      placeholder="Katt!"
    />
             <Space h="md" />
        <Button variant="gradient"
      gradient={{ from: 'indigo', to: 'cyan', deg: 90 }} type="submit" onClick={close}>Mentés</Button>
      </Modal>
      <Button onClick={open} variant="gradient"
      gradient={{ from: 'indigo', to: 'cyan', deg: 90 }}>Adatok módosítása</Button>
